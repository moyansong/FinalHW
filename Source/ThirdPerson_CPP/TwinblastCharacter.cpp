...
void ATwinblastCharacter::Fire()
{
	...
	if (World)
	{
		...
		if (!bIsCrouched)
		{
			HitLocation.Z += Distance / 10000.f * (LeftGunLocation.Z - RefLocation.Z) * Weapon->RecoilForce;
		}
...
void ATwinblastCharacter::MultcastRPCWeaponChange_Implementation()
{
	...
	if (WeaponClassArray[CurrWeaponIndex] != nullptr)
	{
		...
		Weapon->CollisionComponent->SetSphereRadius(0.f);
...
void ATwinblastCharacter::ServerReload_Implementation()
{
	Reload();
}
